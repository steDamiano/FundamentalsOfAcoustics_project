...
%peaks in the sampled note
[peaks, bins] = findpeaks(abs(y_fft), 'MinPeakDistance', 60 * Nfft / Fs, 'MinPeakHeight', 10);

%plot fft and peaks found from audio 
plot(f(1:ceil(2500*Nfft / Fs)), abs(y_fft(1 : ceil(2500 * Nfft/Fs))));
hold on;
stem(f(bins(1:7)), peaks(1:7), 'or');


%peaks computed with formula f = n* f0 -> f0 = 65
fund = 260;
ideal_harmonics = zeros(1,7);
for i=1:7
    ideal_harmonics(i) = i * fund;
end

%plot of ideal harmonics
stem(f(ideal_harmonics) * Nfft / Fs, peaks(1:7), 'og')
